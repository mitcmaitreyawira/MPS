...
let AuthService = AuthService_1 = class AuthService {
    ...
    async validate(nisn, password) {
        ...
        if (user) {
            ...
            this.logger.error(`User has password: ${!!user.password}`);
            this.logger.error(`Password hash: ${user.password}`);
        }
        else {
        ...
        this.logger.error(`About to compare: "${trimmedPassword}" with hash "${user.password}"`);
    ...
    sanitize(user) {
        if (!user) {
            ...
            throw new common_1.UnauthorizedException('User data not available');
        }
        if (!user._id) {
